...
class Trainer:
    ...
    def train_multi_event(self,
        ...
        **kwargs,
        ...
        if val_set is not None:
            # if pbc:
            #     tensor_val = torch.tensor(val_set.values)
            # else:
            tensor_val = torch.tensor(val_set[0].values)
            tensor_y_val = dict()
            for risk in range(self.model.config.num_event):
                # duration时期，proportion在该时间段内的占比
    ...
    def train_multi_event_pbc(self,
        train_set,
        val_set=None,
        batch_size=64,
        epochs=100,
        ...
        **kwargs,
        ...
        if val_set is not None:
            # if pbc:
            tensor_val = torch.tensor(val_set[0])
            # else:
            #     tensor_val = torch.tensor(val_set[0].values)
            tensor_y_val = dict()
            for risk in range(self.model.config.num_event):
                # duration时期，proportion在该时间段内的占比
                tensor_y_val["risk_{}".format(risk)] = torch.tensor(val_set[1][:, [0,risk + 1, 3]]).cuda()
        ...
        for epoch in range(epochs):
            ...
            df_train = train_set[0].copy()
            df_y_train = train_set[1].copy()
            np.random.shuffle(df_train)
            np.random.set_state(state)
            np.random.shuffle(df_y_train)

            tensor_train = torch.tensor(df_train)
            tensor_y_train = {}
            for risk in range(self.model.config.num_event):
                tensor_y_train["risk_{}".format(risk)] = torch.tensor(df_y_train[:, [0, risk + 1, 3]])
            ...
            for batch_idx in range(num_train_batch):
                ...
                batch_x_cat = batch_train[:, :, :self.model.config.num_categorical_feature].float()
                # batch_x_cat = batch_x_cat.to(torch.int64)
                batch_x_num = batch_train[:, :,  self.model.config.num_categorical_feature:].float()
    ...
    def fit(self, 
        ...
        **kwargs,
        ...
        if self.model.config.num_event == 1:
            ...
            if pbc:
                return self.train_multi_event_pbc(
                    train_set=train_set,
                    val_set=val_set,
                    batch_size=batch_size,
                    epochs=epochs,
