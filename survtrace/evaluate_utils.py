...
class Evaluator:
    def __init__(self,  train_time_event, test_time_event):
        '''the input duration_train should be the raw durations (continuous),
        NOT the discrete index of duration.
        '''
        self.train_time_event = train_time_event
        self.test_time_event = test_time_event
    ...
    def eval_multi(self, model, test_set, val_batch_size=10000):
        ...
        for risk_idx in range(model.config.num_event):
            durations_train, events_train = self.train_time_event[:,0], self.train_time_event[:,1] #get_target(df_train_all, risk_idx)
            durations_test, events_test = self.test_time_event[:,0],self.test_time_event[:,1] #get_target(df_y_test, risk_idx)
