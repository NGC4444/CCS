...
def compute_classification_loss(out_cf_class, target_class, device):
    ...
    """
    # out_cf_class1 = (1 - out_cf_class )%2
    # out_cf_class2 = (out_cf_class + 1)%3
    # out_cf_class3 = (out_cf_class + 2)%3
    # target_tensor = torch.full((out_cf_class.shape), target_class).to(device)
    dist_pred_target = torch.sqrt(torch.pow((out_cf_class - target_class), 2)).to(device) 
    # dist_pred_target2 = torch.sqrt(torch.pow((out_cf_class2 - target_class), 2)).to(device) 
    # dist_pred_target3 = torch.sqrt(torch.pow((out_cf_class3 - target_class), 2)).to(device) 
    return dist_pred_target# * 0.8 + dist_pred_target2 * 0.1 + dist_pred_target3 * 0.1
