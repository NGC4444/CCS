...
def plot_cumulative_dynamic_auc(test_lable, out_risk, name, color=None):
    ...
    # for i in range(3):
    fpr, tpr, threshold = metrics.roc_curve(test_lable, out_risk)
    roc_auc = metrics.auc(fpr, tpr)
    
    # plt.plot(fpr, tpr, color[i], label = 'Val AUC  %0.3f' % roc_auc)
    plt.plot(fpr, tpr, colors[1], label = ' AUC '+' %0.3f'% roc_auc)
...
class SurvivalLoss(nn.Module):
        def __init__(self,beta) -> None:
            super().__init__()
            self.beta = torch.Tensor(beta).t().double()
        def forward(self,cf,source):
            # self.beta = torch.Tensor(self.beta)
            p_cf = torch.exp(cf@self.beta)
            p_sr = torch.exp(source@self.beta)
            '''
            p_cf < p_sr
            '''
            # mse = torch.nn.MSELoss(reduction="none")
            loss = torch.sqrt(torch.square(torch.mean(p_cf) - torch.mean(p_sr)))
            return loss
            # print()
...
class CounterfactualTimeGAN(nn.Module):
    ...
    def build_model(self, args, device, num_features, bidirectional=True, \
        hidden_dim_generator=32, layer_dim_generator=2, hidden_dim_discriminator=128, layer_dim_discriminator=2, classifier_model_name=None):
        ...
        print(f'Classifier number of trainablqe parameters: {count_model_parameters(classifier, trainable_only=True)}')
        ...
        discriminator_optim = Adam(discriminator.parameters(), lr=args.dislr, betas=(0.5, 0.999))
    ...
    def _compute_classification_loss(self, out_cf, target_class):
        ...
        dist_pred_target = crossentropy(out_cf, target_class[:out_cf.shape[0]]).to(self.device)
    ...
    def _train_step(self, real_input, real_labels, generator_input,survival_model):
        ...
        #风险函数损失
        cov = survival_model.params_
        _sloss = SurvivalLoss(cov)

        sloss = _sloss(cf.reshape(-1,25),generator_input.reshape(-1,25))

        out_cf_class = torch.argmax(out_cf, dim=1).to(self.device).reshape(-1,1).float()
        target_class = torch.argmax(real_labels, dim=1).to(self.device).reshape(-1,).long()

        cls_loss = self._compute_classification_loss(out_cf, target_class)
        similarity_loss = compute_similarity_loss(deltas,generator_input)
        sparsity_loss = compute_sparsity_loss(deltas)
        jerk_loss = compute_jerk_loss(deltas, self.device)
        sloss = _sloss(cf.reshape(-1,25),real_input.reshape(-1,25))

        fake_out = self.discriminator(cf)
        adv_loss = self._compute_adversarial_loss(fake_out)
        generator_loss = self._compute_generator_loss(adv_loss, cls_loss, similarity_loss, sparsity_loss, jerk_loss) #+ sloss 

        print(f'[Avg Classification Loss: {np.mean(cls_loss.detach().cpu().numpy()):.5f}] \
            [Avg Adversarial Loss: {np.mean(adv_loss.detach().cpu().numpy()):.5f}]\
                 [Avg Similarity Loss: {np.mean(similarity_loss.detach().cpu().numpy()):.5f}] \
                    [Avg Sparsity Loss: {np.mean(sparsity_loss.detach().cpu().numpy()):.5f}] \
                        [Avg Jerk Loss: {np.mean(jerk_loss.detach().cpu().numpy()):.5f}] \
                            [Survival Loss: {sloss.detach().cpu().numpy():.5f}]')# 
        ...
        'Survival Loss':sloss.detach().cpu().item()
        }

        return cf, generator_loss, discriminator_loss, out_cf_class, losses


    def train(self, train_dl=None, target_input=None,survival_model=None):
        """Train generator and discriminator over batches and epochs.

        Args:
            train_dl: input
            target_input: target domain feature
            target_lable: taget lable
        """
        ...
        # pre = DeepRecurrentSurvivalMachines(k = param['k'][0],
        #                             distribution = param['distribution'][0],
        #                             hidden = param['hidden'][0],  
        #                             typ = param['typ'][0],
        #                             layers = param['layers'][0])
        # pre._gen_torch_model(25,"Adam",1)
        # pre.load_state_dict(torch.load("pretrain_survival.pth"))
        for epoch in range(self.args.num_epochs):
            ...
            for batch, (X, y) in enumerate(train_dl):
                for bt, (X_tgt,y_tgt) in enumerate(target_input):
                    batches += 1
                    generated_input, generator_loss, discriminator_loss, out_cf_classes, losses = \
                        self._train_step(real_input=X_tgt, real_labels=y_tgt, generator_input=X,survival_model=survival_model)            
                    print(f'[Epoch: {epoch+1}/{self.args.num_epochs}] [Batch: {batches}] [D Loss: {discriminator_loss}] [G Loss: {generator_loss}] \n')

                    sequences_per_epoch.append(generated_input.detach())
                    target_samples_per_epoch.append(X.detach())
                    query_samples_per_epoch.append(X.detach())
                    query_labels_per_epoch.append(X.detach())
                    ...
                    target_input.prefetch_factor = 0
                    break


            train_loss_per_epoch_generator.append(train_generator_loss / batches)
            train_loss_per_epoch_discriminator.append(train_discriminator_loss / batches)
        ...
        '''
        cls_loss,adv_loss, sim_loss, sp_loss, jerk_loss, G_loss, D_loss, SLoss = [],[],[],[],[],[],[],[]
        for i in range(len(losses_overall)):
            for j in range(len(losses_overall[i])):
                cls_loss.append(np.mean(losses_overall[i][j]['Classification Loss']))
                adv_loss.append(np.mean(losses_overall[i][j]['Adversarial Loss']))
                sim_loss.append(np.mean(losses_overall[i][j]['Similarity Loss']))
                sp_loss.append(np.mean(losses_overall[i][j]['Sparsity Loss']))
                jerk_loss.append(np.mean(losses_overall[i][j]['Jerk Loss']))
                G_loss.append(np.mean(losses_overall[i][j]['Generator Loss']))
                D_loss.append(np.mean(losses_overall[i][j]['Discriminator Loss']))
                SLoss.append(np.mean(losses_overall[i][j]['Survival Loss']))
        # label_X = range(len(SLoss))
        # plt.figure()
        # plt.plot(label_X,np.array(SLoss), label='Survival Loss')
        # # plt.plot(label_X,epocheval, label='Eval')
        # plt.title("Survival loss per epoch ")
        # plt.xlabel('Epoch')
        # plt.ylabel('Loss')
        # plt.legend()
        # plt.savefig("SurvivalLoss.png")
        # plt.clf()

        # plt.figure()
        # plt.plot(label_X,np.array(G_loss), label='Generator Loss')
        # # plt.plot(label_X,epocheval, label='Eval')
        ...
        # plt.figure()
        # plt.plot(label_X,np.array(jerk_loss), label='Jerk Loss')
        # # plt.plot(label_X,epocheval, label='Eval')
        # plt.title("Jerk loss per epoch ")
        # plt.xlabel('Epoch')
        # plt.ylabel('Loss')
        # plt.legend()
        ...
        # plt.figure()
        # plt.plot(label_X,np.array(sp_loss), label='Sparsity Loss')
        # # plt.plot(label_X,epocheval, label='Eval')
        # plt.title("Sparsity loss per epoch ")
        # plt.xlabel('Epoch')
        # plt.ylabel('Loss')
        # plt.legend()
        ...
        # plt.figure()
        # plt.plot(label_X,np.array(sim_loss), label='Similarity Loss')
        # # plt.plot(label_X,epocheval, label='Eval')
        # plt.title("Similarity loss per epoch ")
        # plt.xlabel('Epoch')
        # plt.ylabel('Loss')
        # plt.legend()
        ...
        # plt.figure()
        # plt.plot(label_X,np.array(adv_loss), label='Adversarial Loss')
        # # plt.plot(label_X,epocheval, label='Eval')
        # plt.title("Adversarial loss per epoch ")
        # plt.xlabel('Epoch')
        # plt.ylabel('Loss')
        # plt.legend()
        ...
        # plt.figure()
        # plt.plot(label_X,np.array(cls_loss), label='Classification Loss')
        # # plt.plot(label_X,epocheval, label='Eval')
        ...
        # plt.figure()
        # plt.plot(label_X,np.array(D_loss), label='Discriminator Loss')
        # # plt.plot(label_X,epocheval, label='Eval')
    ...
    def test(self, test_dl,tgt_data, testdoc):
        ...
        with torch.no_grad():
            for batch, (X, y) in enumerate(test_dl):
                ...
                # freeze_indices = [1]
                remaining_indices = [x for x in range(len(feature_mapping)) if x not in [freeze_indices]]
                # remaining_indices = [x for x in range(len(feature_mapping)) if x not in [2,3,24]]

                if not feature_mapping:
                    remaining_indices = range(0, 25)
                remaining_indices = [x for x in range(25) if x not in [2,3,24]]
                deltas = self.generator(X)
                
                deltas_with_freeze = torch.zeros_like(X)
                deltas_with_freeze[:,:,remaining_indices] = deltas[:,:,remaining_indices]
                for i in range(deltas.size(0)):
                    np.save("deltas/batch{}-sample{}.npy".format(str(batch),str(i)),deltas_with_freeze[i].detach().cpu().numpy())

                cf = deltas + X
                if dataplt != []:
                    dataplt =  np.concatenate([dataplt, X.numpy().reshape(current_batchsize,-1), cf.numpy().reshape(current_batchsize,-1)],axis=0)
                    lable = np.concatenate([lable, y.numpy(),y.numpy() + 1], axis=0)
                else:
                    dataplt =  np.concatenate([X.numpy().reshape(current_batchsize,-1), cf.numpy().reshape(current_batchsize,-1)],axis=0)
                    lable = np.concatenate([y.numpy(),y.numpy() + 1], axis=0)
                # dataplt = cf.numpy().reshape(17,-1)
                # lable = y_generator_input[:,0]

                out_cf = self.classifier(cf).to(self.device)
                cls_roc+=torch.softmax(out_cf,1)[:,0].detach().cpu().numpy().tolist()
                out_cf_class = torch.argmax(out_cf, dim=1)

                # cls_roc.append(out_cf.detach().cpu().numpy())

                cls_loss = compute_classification_loss(out_cf_class, y.double(), device)
                ...
                losses = {
                'Classification Loss': cls_loss.mean().detach().cpu().item(),
                'Adversarial Loss': adv_loss.mean().detach().cpu().item(),
                'Similarity Loss': similarity_loss.mean().detach().cpu().item(),
                'Sparsity Loss': sparsity_loss.mean().detach().cpu().item(),
                'Jerk Loss': jerk_loss.mean().detach().cpu().item(),
                'Generator Loss': generator_loss.mean().detach().cpu().item(),
                }
            ...
            for batch, (X,y) in enumerate(tgt_data):
                y = torch.argmax(y,dim=1)
                dataplt =  np.concatenate([dataplt, X.numpy().reshape(X.shape[0],-1)],axis=0)
                lable = np.concatenate([lable, y.numpy()], axis=0)
            # plot_cumulative_dynamic_auc(np.zeros((len(cls_roc),1)),np.array(cls_roc),'cfgan') 
            #draw---画图查看分布情况
            pca = PCA(n_components=2)
            pca = pca.fit(dataplt)
            x_dr = pca.transform(dataplt)
            plt.figure()#创建一个画布
            plt.scatter(x_dr[lable==0,0],x_dr[lable==0,1],c="red",label = 'target')
            plt.scatter(x_dr[lable==1,0],x_dr[lable==1,1],c = "grey",label = 'query')
            # plt.scatter(x_dr[lable==2,0],x_dr[lable==2,1],c="blue",label = 'lable=2,raw')
            plt.scatter(x_dr[lable==2,0],x_dr[lable==2,1],c="c",label = 'generated')
            # plt.scatter(x_dr[lable==0,0],x_dr[lable==0,1],c="red",label = 'target')
            plt.plot(np.mean(x_dr[lable==0,0]),np.mean(x_dr[lable==0,1]),marker='+',color='fuchsia',label='target center')
            plt.plot(np.mean(x_dr[lable==2,0]),np.mean(x_dr[lable==2,1]),marker='+',color='cyan',label='generated center')
            dis = np.sqrt(np.square(np.mean(x_dr[lable==0,0]) - np.mean(x_dr[lable==2,0])) + np.square(np.mean(x_dr[lable==0,1]) - np.mean(x_dr[lable==2,1])))
            print("distance: {}".format(str(dis)))
            # plt.scatter(x_dr[lable==3,0],x_dr[lable==3,1],c="green",label = 'lable=1,fake')
            # plt.scatter(x_dr[lable==5,0],x_dr[lable==5,1],c="orange",label = 'lable=2,fake')
            # for i in range(len(x_dr[lable==0])):
            #     plt.plot(x_dr[lable==1][i],x_dr[lable==2][i],c='black')
            plt.legend()#显示图例
            # plt.title("PCA of PBC2 dataset")#显示标题
            plt.title("SPARCE | PBC2")#显示标题
            plt.savefig("figure/pbc2-SPARCE.png")
            plt.show()
            ...
            np.save('res/testdoc.npy',testdoc)
            return testdoc