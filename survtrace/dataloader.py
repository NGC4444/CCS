...
def load_data(pathToData, fileFormat, replicate_labels_indicator=False):
    ...
    """
    X_train, y_train,t_train = np.load(os.path.join(pathToData, 'X_train' + fileFormat)), np.load(os.path.join(pathToData, 'y_train'+ fileFormat)), np.load(os.path.join(pathToData, 't_train'+ fileFormat))
    X_val, y_val,t_val= np.load(os.path.join(pathToData, 'X_val'+ fileFormat)), np.load(os.path.join(pathToData, 'y_val'+ fileFormat)), np.load(os.path.join(pathToData, 't_val'+ fileFormat))
    X_test, y_test,t_test = np.load(os.path.join(pathToData, 'X_test'+ fileFormat)), np.load(os.path.join(pathToData, 'y_test'+ fileFormat)), np.load(os.path.join(pathToData, 't_test'+ fileFormat))
...
def _padding_nan_with_zero(data):
  return torch.where(
    torch.isnan(data),
    torch.full_like(data, 0),
    data
  )
...
def get_dataloader(X_train, y_train, batchsize,shuffle=False):
    """Create training, validation and testing dataloaders for numpy arrays.
    ...
    # pass datasets to dataloaders (wraps an iterable over the dataset)
    train_dl = DataLoader(train_ds, batchsize, shuffle=shuffle)
