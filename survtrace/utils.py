...
def pata_discre(pata, cut):
    ...
    '''
    y = np.zeros((pata.shape[0], 4))
    dif = np.diff(cut)
    for i in range(pata.shape[0]):
        # cut: 0, 1/4, 1/2, 3,4, 1
        periods = bisect_left(cut, pata[i, 1])
        y[i][0] = periods - 1#pata[i, 1] 
        if(pata[i,2] == 1):
            y[i][1] = 1
        elif(pata[i,2] == 2):
            y[i][2] = 1
        y[i][3] = (pata[i,1] - cut[int(periods)-1]) / dif[int(periods)-1]
    return y